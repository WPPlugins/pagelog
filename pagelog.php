...
global $wpdb;
	...
	if ( isset( $_POST['submit'] ) ) {
		...
		if ( isset( $_POST['add_shortcodes_pages'] ) ) {
			$pages = $wpdb->get_results( "SELECT `ID`, `post_content` FROM `" . $wpdb->prefix . 'posts' . "` WHERE `post_status` = 'publish' AND `post_type` = 'page'", ARRAY_A );
			if ( $pages ) {
				foreach( $pages as $page ) {
					$txt = $page['post_content'];
					if ( strpos( $page['post_content'], '[pagelog]' ) === false ) {
						$txt .= '[pagelog]';
						$wpdb->query( $wpdb->prepare( "UPDATE `" . $wpdb->prefix . 'posts' . "` SET `post_content` = %s WHERE `ID` = %s", $txt, $page['ID'] ) );
					}
				}
			}
		}
		if ( isset( $_POST['add_shortcodes_posts'] ) ) {
			$pages = $wpdb->get_results( "SELECT `ID`, `post_content` FROM `" . $wpdb->prefix . 'posts' . "` WHERE `post_status` = 'publish' AND `post_type` = 'post'", ARRAY_A );
			if ( $pages ) {
				foreach( $pages as $page ) {
					$txt = $page['post_content'];
					if ( strpos( $page['post_content'], '[pagelog]' ) === false ) {
	...
	if ( $save_period ) {
		$wpdb->query( $wpdb->prepare( "DELETE FROM `" . PAGELOG . "` WHERE `timestamp` < %d", time() - $save_period ) );
	}
	...
	'<script type="text/javascript" >
		jQuery(document).ready(function(){
			...
			// Go to the first checked tab
			var tab, i;
			i=1;
			while ( i < 7 ) {
				if ( jQuery( "#tab"+i+"check" ).prop("checked") == true ) {
					jQuery( "#label"+i ).click();
					return;
				}
				i++;
			}
	...
	echo 		'<ul class="tabs" style="margin-bottom:0;" >' .
					( get_option( 'pagelog_tab1check', true ) ? '<li><a id="label1" href="#tab1" >' . esc_html( __( 'By frequency', 'pagelog' ) ) . '</a></li>' : '' ) .
					( get_option( 'pagelog_tab2check', true ) ? '<li><a id="label2" href="#tab2" >' . esc_html( __( 'By Permalink', 'pagelog' ) ) . '</a></li>' : '' ) .
					( get_option( 'pagelog_tab3check', true ) ? '<li><a id="label3" href="#tab3" >' . esc_html( __( 'Most recent', 'pagelog' ) ) . '</a></li>' : '' ) .
					( get_option( 'pagelog_tab4check', true ) ? '<li><a id="label4" href="#tab4" >' . esc_html( __( 'By user', 'pagelog' ) ) . '</a></li>' : '' ) .
					( get_option( 'pagelog_tab5check', true ) ? '<li><a id="label5" href="#tab5" >' . esc_html( __( 'By IP', 'pagelog' ) ) . '</a></li>' : '' ) .
					( get_option( 'pagelog_tab6check', true ) ? '<li><a id="label6" href="#tab6" >' . esc_html( __( 'History', 'pagelog' ) ) . '</a></li>' : '' ) .
					'<li><a id="label9" href="#tab9" >' . esc_html( __( 'Settings', 'pagelog' ) ) . '</a></li>' .
				'</ul>';
	...
	$log_page_ids = $wpdb->get_col( "SELECT DISTINCT `page` " .
									"FROM `" . PAGELOG . "` " .
									( $pagelog_display_period ? "WHERE `timestamp` > " . ( time() - $pagelog_display_period ) . " " : "" )
									);
	...
	if ( get_option( 'pagelog_tab1check', true ) ) {
	...
	if ( get_option( 'pagelog_tab2check', true ) ) {
	...
	if ( get_option( 'pagelog_tab3check', true ) ) {
	...
	if ( get_option( 'pagelog_tab4check', true ) ) {
	...
	echo '<div id="tab5" style="display:none;" >';
	if ( get_option( 'pagelog_tab5check', true ) ) {
		pagelog_by_ip( $pages );
	}
	else {
		pagelog_dummy();
	...
	// Tab 6: History
	echo '<div id="tab6" style="display:none;" >';
	if ( get_option( 'pagelog_tab6check', true ) ) {
	...
	// Tab 9: Settings
	echo '<div id="tab9" style="display:none;max-width:600px;" >';
		pagelog_settings();
	echo '</div>';
...
static $been_here;
	...
	if ( ! $been_here ) {
		$wpdb->query( $wpdb->prepare( "INSERT INTO `" . PAGELOG . "` (
														`page`,
														`timestamp`,
														`ip`,
														`user`
	...
	if ( $atts['show'] == 'yes' || ( $atts['show'] == 'login' && is_user_logged_in() ) ) {
...
function pagelog_settings() {
	...
	'<form method="post" onsubmit="' . admin_url( 'tools.php?page=pagelog' ) . '" >';
		...
		echo
		'<table style="border:1px solid #72777c;" >' .
			'<tbody>' .
				'<tr>' .
					'<td>' .
